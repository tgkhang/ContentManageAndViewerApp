...
export class ContentsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  @WebSocketServer()
  server: Server;
  ...
  handleJoinRoom(client: Socket, contentId: string) {
  ...
  handleLeaveRoom(client: Socket, contentId: string) {
    client.leave(`content-${contentId}`);
    this.logger.log(`Client ${client.id} left room: content-${contentId}`);
  }

  sendContentUpdate(content: ContentDocument) {
    // Broadcast to all clients in the specific content room
    this.server.to(`content-${content._id}`).emit('contentUpdated', content);
